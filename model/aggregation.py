...
class NetVLAD(nn.Module):
    ...
    def forward(self, x):
        ...
        x_flatten = x.view(N, D, -1)        # shape: N, D, HW
    ...
    def initialize_netvlad_layer(self, args, cluster_ds, backbone):
        if args.backbone.startswith('swin'):
            descriptors_num = 20000
            # for swin choose 40 swin_output(16,49,768)->(16,40,768) :sample 100 in res18_output(16,196,256)->(16,100,256)
        ...
        random_sampler_query = SubsetRandomSampler(np.random.choice(range(cluster_ds.database_num,
                                                                          cluster_ds.database_num+cluster_ds.queries_num),
                                                                    int(images_num), replace=False))
        random_sampler_database = SubsetRandomSampler(np.random.choice(range(cluster_ds.database_num),
                                                                       int(images_num), replace=False))

        random_dl_query = DataLoader(dataset=cluster_ds, num_workers=args.num_workers,
                                batch_size=args.infer_batch_size, sampler=random_sampler_query)

        random_dl_database = DataLoader(dataset=cluster_ds, num_workers=args.num_workers,
                                        batch_size=args.infer_batch_size, sampler=random_sampler_database)

        with torch.no_grad():
            backbone = backbone.eval()
            logging.debug("Extracting features to initialize NetVLAD layer")

            descriptors_q = np.zeros(shape=(descriptors_num, args.features_dim), dtype=np.float32)
            for iteration, (inputs, idx) in enumerate(tqdm(random_dl_query, ncols=100, desc='Initializing NetVLAD [Query]')):
                ...
                if args.backbone.startswith('swin') or args.backbone.startswith('vit'):
                    # swin don't need to reshape
                    norm_outputs = F.normalize(outputs, p=2, dim=2)
                    image_descriptors = norm_outputs
                else:
                ...
                for ix in range(image_descriptors.shape[0]):
                    ...
                    descriptors_q[startix:startix + descs_num_per_image, :] = image_descriptors[ix, sample, :]

            descriptors_db = np.zeros(shape=(descriptors_num, args.features_dim), dtype=np.float32)
            for iteration, (inputs, idx) in enumerate(tqdm(random_dl_database, ncols=100, desc='Initializing NetVLAD [Database]')):
                rand_patch_idx = np.random.choice(args.split_nums, 1, replace=False)
                inputs_width = inputs.shape[3]
                stride = int(inputs_width / args.split_nums)
                left = int(stride * rand_patch_idx)
                right = int(stride * rand_patch_idx + args.resize[1])
                if left < right <= inputs_width:
                    inputs = inputs[:, :, :, left:right]
                elif left < inputs_width < right:
                    inputs = torch.cat((inputs[:, :, :, left:], inputs[:, :, :, :right - inputs_width]), dim=-1)
                ...
                for ix in range(image_descriptors.shape[0]):
                    sample = np.random.choice(image_descriptors.shape[1], descs_num_per_image, replace=False)
                    startix = batchix + ix * descs_num_per_image
                    descriptors_db[startix:startix + descs_num_per_image, :] = image_descriptors[ix, sample, :]

            descriptors = np.concatenate((descriptors_q,descriptors_db),axis=0)
