...
def _download_map_image(min_lat=45.0, min_lon=7.6, max_lat=45.1, max_lon=7.7, size=2000):
    ...
    static_map = StaticMap(size, size, url_template='https://api.mapbox.com/styles/v1/mapbox/satellite-v9/tiles/{z}/{x}/{y}?access_token=<pk.eyJ1IjoicGFub3ZwciIsImEiOiJjbGV6NjR5Y2swNHoyM3JtcHh6b285cm5iIn0.cP-yJHjEYSnmz7ckNcIBXw>')
...
def _create_map(coordinates, colors=None, dot_sizes=None, legend_names=None, map_intensity=0.6):
    ...
    if legend_names != None:
        plt.legend(scatters, legend_names, scatterpoints=10000, loc='upper left',
...
def _get_coordinates_from_dataset(dataset_folder, extension="jpg"):
    ...
    for image_path in images_paths:
        ...
        gps_lat = image_path.split("@")[6]
        gps_lon = image_path.split("@")[7] if not image_path.split("@")[7].endswith(f'{extension}') \
                                            else image_path.split("@")[7][:-len(extension)-1]
...
def build_map_from_dataset(dataset_folder, dot_sizes=None):
    ...
    grouped_gps_coords = _get_coordinates_from_dataset(join(dataset_folder))
    ...
    io.imsave(join(dataset_folder, f"map_{dataset_name}_final.png"), map_img)

def main():
    dataset_folder = '/home/zafirshi/Datasets/YQ360'
    build_map_from_dataset(dataset_folder)
